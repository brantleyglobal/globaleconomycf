...
import { CommonInputProps, InputBase, SIGNED_NUMBER_REGEX } from "~~/components/globalEco";
...
function displayValueToEtherValue(usdMode: boolean, displayValue: string, nativeCurrencyPrice: number) {
  if (usdMode && nativeCurrencyPrice) {
    ...
    if (Number.isNaN(parsedDisplayValue)) {
      ...
      // Compute the GBD value if a valid number.
...
/**
 * Input for GBD amount with USD conversion.
 *
 * onChange will always be called with the value in GBD
 */
  ...
  return (
    <InputBase
      ...
      suffix={
        ...
        >
          <button
            className="btn btn-primary rounded-md h-[2.2rem] min-h-[2.2rem]"
